...
- [2. 系統架構與流程](#2-系統架構與流程)
  - [2.1 認證機制](#21-認證機制)
  - [2.2 Session 管理](#22-session-管理)
  - [2.3 GCP 資源生命週期](#23-gcp-資源生命週期)
  - [2.4 核心時序流程](#24-核心時序流程)
  - [2.5 API 端點總覽](#25-api-端點總覽)
- [3. Agora Media Push 轉碼模式設定](#3-agora-media-push-轉碼模式設定)
  - [3.1 無轉碼直推模式（預設，POC 建議）](#31-無轉碼直推模式預設poc-建議)
  - [3.2 轉碼模式（可選）](#32-轉碼模式可選)
  - [3.3 未來升級參考配置（1080p）](#33-未來升級參考配置1080p)
- [4. Session 資料模型](#4-session-資料模型)
- [5. 技術棧 (Tech Stack)](#5-技術棧-tech-stack)
- [6. 注意事項](#6-注意事項)
...
## 3. Agora Media Push 轉碼模式設定

### 3.1 無轉碼直推模式（預設，POC 建議）
...
- `doc/spec.md`（本檔）：將 3.2 節的轉碼設定替換為 1080p 數値，並移除本節。

---

## 4. Session 資料模型

詳見實作：`server/internal/model/session.go`
