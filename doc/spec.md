...
- `server/internal/provider/agora_media_push.go`：更新推流解析度與碼率參數。
- `server/internal/provider/gcp_livestream.go`：更新 Channel 的 ElementaryStream 轉碼配置（`videoStream` 的 `heightPixels`、`bitrateBps`）。
- `doc/spec.md`（本檔）：將 4.1 節替換為 1080p 數值，並移除本節。
